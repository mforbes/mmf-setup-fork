    ...
    * Suppress output in an easy way.
    ...
    def dispatch(self, cmd, fout=None, ferr=None):
        ...
        if API:
            ...
            if isinstance(cmd, str):
                cmd = shlex.split(cmd)
            cmd = list(map(b, cmd))
            self.result = self.client.runcommand(cmd, inchannels, outchannels)
            if fout is None:
    ...
    def revert(self, rev, files=(), all=True, nobackup=True):
        ...
        if API:
            ...
            return self.dispatch(
                "revert {} -r {} {}".format(" ".join(opts), rev, " ".join(files))
            )
        else:
    ...
    def update(self, rev, clean=True, quiet=True):
        ...
        if True or API:
            ...
            if clean:
                opts.append("-C")
            return self.dispatch("update {} {}".format(" ".join(opts), rev), ferr=False)
        else:
            try:
    ...
    def checkpoint(self, force=False):
        ...
        for bookmark in [self.tags["parent"], self.tags["checkpoint"]]:
        ...
        try:
            if API:
                ...
                try:
                    ...
                    try:
                        self.client.tag(
                            names=self.tags["new"].encode(), local=True, force=True
                        )
                    except hglib.error.CommandError:
                        pass
    ...
    def config(self, name, default="", section="nbclean"):
        ...
        if API:
            ...
            res = dict(
                (_key.decode(), _value.decode()) for _section, _key, _value in config
            )
...
@_cmd(
    opts=[
        (b"", b"clean-all", False, _("clean all managed .ipynb files")),
        (b"f", b"force", False, _("force removal of managed bookmarks etc.")),
    ]
)
...
@_cmd(
    opts=commands.table[b"commit|ci"][1]
    + [
        (b"", b"clean-all", False, _("clean all managed .ipynb files")),
        (b"b", b"branch", b"", _("commit output to this branch (create if needed)")),
    ],
...
@_cmd(
    opts=commands.table[b"update|up|checkout|co"][1],
...
def cupdate(state, *pats, **opts):
    ...
    auto_commits = sorted(
        [
            _r
            for _r in state.client.log("children(.)")
...
@_cmd(
    opts=commands.table[b"status|st"][1]
    + [(b"", b"clean-all", False, _("clean all managed .ipynb files"))],
